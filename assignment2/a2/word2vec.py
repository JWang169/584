...
def helpernormRows(x):
    """normalize rows in matrix
    x -- a matrix to be normed

    return:
    x -- matrix with normed rows
    """
    normed = norm(x, axis=1, keepdims=True)
    x /= normed
    return x
...
def naiveSoftmaxLossAndGradient(
    ...
    gradOutsideVecs =  # (N, 1) dot (1, d) -> (N, d)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    # print('centerWordWvectors.shape: ', centerWordVectors.shape) 
    # the shape is 5 * 3, 5 words, 3 dim for each word
